...
function getNames(Presidents) {
  return Presidents.map((president) => president.name); // Use lowercase 'president' for clarity and proper mapping
}
...
function countRepublicanPresidents(presidents) {
  return presidents.reduce(function (acc, curr) {
    if (curr.party === "Republican") acc++;
    return acc;
  }, 0);
}
